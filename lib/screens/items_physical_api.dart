...
class ItemsPhysicalAPI extends StatefulWidget {
  const ItemsPhysicalAPI({Key key}) : super(key: key);

  @override
  _ItemsPhysicalAPIState createState() => _ItemsPhysicalAPIState();
}


class _ItemsPhysicalAPIState extends State<ItemsPhysicalAPI>  with AutomaticKeepAliveClientMixin, TickerProviderStateMixin<ItemsPhysicalAPI>{
  ...
  Future<List<Item>> fetchItemsBased() async {
    final response =
    await dio.get('https://hninsunyein.me/rift_plus/rift_plus/api/getAllPhysicalItems.php');
    //print(response.body);
    if (response.statusCode == 200) {
      itemResponse = response.toString();
      Map<String, dynamic> jsonObject = json.decode(itemResponse);
  ...
  Future<List<Item>> fetchItemsMidtier() async {
    final response =
    await http.get('https://hninsunyein.me/rift_plus/rift_plus/api/getAllPhysicalItems.php');
    //print(response.body);
    if (response.statusCode == 200) {
      Map<String, dynamic> jsonObject = json.decode(response.body);
      Iterable list = jsonObject["mid-tier"];
                          ...
                          Container(
                              ...
                              child: GridView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  print('item       ' + snapshot.data[index].item_name2);
                                ...
                                children: <Widget>[
                                  ...
                                  InkWell(
                                    ...
                                    onTap: () {
                                      Navigator.of(context).push(
                                          MaterialPageRoute(
                                              builder: (context) => SeeAllRoute(item: itemResponse, id: 'mid-tier', name:'MID TIER ITEMS'))
                                      );
                                    },
                                    child: Padding(
                                      padding: const EdgeInsets.only(left: 8),
