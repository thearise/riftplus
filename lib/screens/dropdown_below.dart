...
const double _kMenuItemHeight = 35.0;
...
class _DropdownMenuPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect rect = new Rect.fromLTRB(
        0, top.evaluate(resize)-3, size.width, bottom.evaluate(resize)+3);
      ...
      children.add(new FadeTransition(
        ...
        child: new InkWell(
            child: new Container(
              ...
              child: Padding(
                  padding: const EdgeInsets.only(top: 0.0, bottom: 2.0, right:0),
                  child: FutureBuilder(
                      future: langs,
                      builder: (context, snapshot) {
                        if(snapshot.hasData) {
                          print('below shwe ' + route.items[itemIndex].value.toString() + snapshot.data.toLowerCase() );
                          if(route.items[itemIndex].value.toString().toLowerCase().contains(snapshot.data.toLowerCase())) {
                            return Row(
                              children: [
                                Text(
                                  itemString(route.items[itemIndex].value.toString()),
                                  textScaleFactor: 1,
                                  style: TextStyle(
                                    fontFamily: 'language',
          ...
          new Positioned(
            ...
            child: new Container(
              ...
              decoration: const BoxDecoration(
                  border: Border(
                      bottom:
                      BorderSide(color: AppTheme.secBgColor, width: 0))),
            ),
          ),
        ],
      );
