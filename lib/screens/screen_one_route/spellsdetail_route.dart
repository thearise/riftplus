...
class SpellsDetailRoute extends StatefulWidget {
  const SpellsDetailRoute({Key key, this.id, this.image, this.name, this.desc, this.cooldown, this.type, this.sugg, this.video}) : super(key: key);
  final String id;
  final String image;
  final String name;
  final String desc;
  ...
  @override
  _SpellsDetailRouteState createState() => _SpellsDetailRouteState();
}

class _SpellsDetailRouteState extends State<SpellsDetailRoute>  with TickerProviderStateMixin{
  Future<Ads> ads;
  Dio dio;
  ...
  Future<Ads> fetchAds() async {
    final response =
    await dio.get('https://hninsunyein.me/rift_plus/rift_plus/api/spellAds.php');
  ...
  Future<Infos> fetchDetInfos(id) async {
    final response =
    await dio.get('https://hninsunyein.me/rift_plus/rift_plus/api/getSpellDetailsById.php?id='+id.toString());
...
//                      ),
                          ...
                          Padding(
                            padding: const EdgeInsets.only(top: 15, left: 20, right: 20, bottom: 5.0),
                            child: Row(
                              children: <Widget>[
                                Expanded(
                                  child: Text(
                            ...
                            builder: (context, AsyncSnapshot<List<dynamic>> snapshot) {
                              if (snapshot.hasData) {
                                return Align(
                                  alignment: Alignment.centerLeft,
                                  child: Padding(
                                    padding: const EdgeInsets.only(left: 20.0, top:10.0, right: 20.0, bottom:0.0),
                                    child: Container(
                                      child: RichText(
                                          text: modifyDesc(snapshot.data[1]=='_en'?snapshot.data[0].desc_en.toString():snapshot.data[0].desc.toString())
                                      ),
                                    ),
                                  ),
                                );
                          ...
                          Padding(
                            padding: const EdgeInsets.only(top: 0, left: 20, right: 20, bottom: 0.0),
                            child:  widget.video!=''?VideoPlayerCust(widget.video):Container(),
                          ),
                          SizedBox(
                            height: 90,
                          )
...
//    return Container();
  ...
  String getBannerAdUnitId() {
    if (Platform.isIOS) {
      return 'ca-app-pub-7741823874181873/2808856274';
