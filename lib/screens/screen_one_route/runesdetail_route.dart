...
class Infos {
  final String rune_desc;
  final String rune_desc_en;
  final String rune_video;

  Infos({this.rune_desc, this.rune_desc_en, this.rune_video});

  factory Infos.fromJson(Map<String, dynamic> json){
    return Infos(
      rune_desc: json['rune_desc'],
      rune_desc_en: json['rune_desc_en'],
      rune_video: json['rune_video']
    );
  }
}

class RunesDetailRoute extends StatefulWidget {
  const RunesDetailRoute({Key key, this.id, this.image, this.name, this.name2, this.pros, this.desc, this.price, this.type, this.level, this.rune, this.video}) : super(key: key);
  final String id;
  final String image;
  final String name;
  final String name2;
  ...
  @override
  _RunesDetailRouteState createState() => _RunesDetailRouteState();
}

class _RunesDetailRouteState extends State<RunesDetailRoute>  with TickerProviderStateMixin{
  Dio dio;
  Future<Infos> infos;
  Future<Ads> ads;
  Future languages;
  ...
  void initState() {
    AppState().increaseGloAdsInt(0.5);
    if(widget.video!='') {
  ...
  Future<Ads> fetchAds() async {
    final response =
    await dio.get('https://hninsunyein.me/rift_plus/rift_plus/api/runeAds.php');
  ...
  Future<Infos> fetchInfos(id) async {
    final response =
    await dio.get('https://hninsunyein.me/rift_plus/rift_plus/api/getRuneDescriptionById.php?id='+id);
                    ...
                    children: [
                      ListView(
                        children: [
                          ...
                          // StreamBuilder(
                          //     stream: Firestore.instance.collection('runes').document('1Nm0CocGp3Sm1wopTqP1').collection(widget.rune).document(widget.id).collection('text').snapshots(),
                          //     builder: (context, snapshot) {
                          //       if(snapshot.data == null) return SizedBox();
                          //       return ItemsPros(snapshot);
                          //     }
                          // ),
                          SizedBox(
                              child: Padding(
                                padding: const EdgeInsets.only(top: 15,left: 0.0, right: 0.0),
                                child: Container(
                                  decoration: BoxDecoration(
                                      ...
                                      Padding(
                                        ...
                                        child: Container(
                                          child: Align(
                                            alignment: Alignment.topLeft,
                                            child: RichText(
                                              text: modifyDesc(snapshot.data[1]=='_en'?snapshot.data[0].rune_desc_en.toString():snapshot.data[0].rune_desc.toString()),
                                              textScaleFactor: 1,
                                            ),
                                          ),
                                        ),
                                      ),
                                      widget.video!=''?VideoPlayerCust(snapshot.data[0].rune_video):SizedBox(height: 80),
                                    ],
                                  );
                                }
                                return Stack(
...
//                Navigator.of(context).push(
//                  MaterialPageRoute(
//                      builder: (context) => RunesDetailRoute(id: record.id,image: record.image, name: record.name, desc: record.desc, price:record.price, type: record.type))
//                  );
//              },
//            ),
//          ),
